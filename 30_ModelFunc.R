beach_choose <- function(beaches,thresh,num_of_folds){ 
  ...
  change_beachnames <- c('North Avenue_Escherichia.coli','Oak Street_Escherichia.coli')
  changed_beachnames <- c('North_Avenue_Escherichia.coli','Oak_Street_Escherichia.coli')
  for(i in 1:length(change_beachnames))
  {
      names(df)[names(df) == change_beachnames[i]] <- changed_beachnames[i]
  }
  ##------------------------------------------------------------------------------
  ## MODEL SETTINGS
